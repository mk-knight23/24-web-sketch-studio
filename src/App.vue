...
const tools = [
  { id: 'select', label: 'Select', path: 'M3 3l7.07 16.97 2.51-7.39 7.39-2.51L3 3z' },
  {
    id: 'pen',
    label: 'Pen',
    path: 'M12 19l7-7 3 3-7 7-3-3z M18 13l-1.5-7.5L2 2l3.5 14.5L13 18l5-5z M2 2l7.07 16.97',
  },
  {
    id: 'eraser',
    label: 'Eraser',
    path: 'M20 20H7L3 16c-.8-.8-.8-2 0-2.8L13.4 2.8c.8-.8 2-.8 2.8 0L21 7.7c.8.8.8 2 0 2.8L13 18',
  },
...
<template>
  <div
    class="h-screen w-screen flex flex-col bg-[var(--studio-bg)] dark:bg-[var(--studio-bg)] overflow-hidden font-sans transition-colors duration-500"
    :class="{ 'light-mode': !isDarkMode }"
  >
    <!-- Top Header -->
    <header
      class="h-20 border-b border-[var(--studio-border)] px-10 flex items-center justify-between bg-[var(--studio-card)] z-50"
    >
        ...
        <nav class="flex items-center space-x-2 bg-white/5 border border-white/5 rounded-2xl p-2">
          ...
          >
            ...
            >
              ...
              <path
                d="M12 1v6m0 6v6m5.25-12.25l-4.24 4.24m0 8.48l-4.24 4.24M1 12h6m6 0h6m-2.25 5.25l-4.24-4.24m0-8.48l-4.24-4.24"
              />
            </svg>
          </RouterLink>
        </nav>
      ...
      <div class="flex items-center space-x-6">
        <button
          @click="toggleTheme()"
          class="p-2.5 rounded-xl hover:bg-white/5 text-slate-500 hover:text-white transition-all"
        ...
        <button
          class="bg-white text-black px-6 py-2.5 rounded-full font-black text-[10px] uppercase tracking-widest hover:bg-[var(--studio-primary)] hover:text-white transition-all flex items-center"
        >
          <svg
            class="mr-2"
            xmlns="http://www.w3.org/2000/svg"
    ...
    <div class="flex-1 flex relative">
        ...
        <div class="glass-toolbar p-3 flex flex-col items-center space-y-4 text-white">
          <div
            ...
            :style="{
              ...
              borderColor: store.strokeColor === color ? 'white' : 'transparent',
            }"
            role="button"
      ...
      <!-- Canvas Stage -->
      <main
        class="flex-1 bg-[var(--studio-canvas)] relative overflow-hidden flex items-center justify-center"
      >
          ...
          >
            ...
            >
              ...
              <template v-for="shape in layer.shapes" :key="shape.id">
                ...
                <v-rect
                  ...
                  :config="{
                    x:
                      (shape.x || 0) + (shape.width || 0) < (shape.x || 0)
                        ? (shape.x || 0) + (shape.width || 0)
                        : shape.x,
                    y:
                      (shape.y || 0) + (shape.height || 0) < (shape.y || 0)
            ...
            <v-layer v-if="store.currentShape">
              ...
              <v-rect
                ...
                :config="{
                  x:
                    (store.currentShape.x || 0) + (store.currentShape.width || 0) <
                    (store.currentShape.x || 0)
                      ? (store.currentShape.x || 0) + (store.currentShape.width || 0)
                      : store.currentShape.x,
                  y:
                    (store.currentShape.y || 0) + (store.currentShape.height || 0) <
                    (store.currentShape.y || 0)
                      ? (store.currentShape.y || 0) + (store.currentShape.height || 0)
                      : store.currentShape.y,
      ...
      <!-- Layers Panel -->
      <aside
        class="w-80 bg-[var(--studio-card)] border-l border-[var(--studio-border)] p-8 flex flex-col space-y-10 z-40 overflow-y-auto custom-scrollbar"
      >
        <div class="space-y-2">
        ...
        <div class="space-y-4 text-white">
          <div class="flex items-center justify-between">
            <span class="text-[10px] font-black uppercase tracking-widest text-slate-600"
              >{{ store.layers.length }} ACTIVE LAYERS</span
            >
            <button
              @click="store.addLayer"
              class="p-2 hover:bg-white/5 rounded-lg text-[var(--studio-primary)] transition-all"
            ...
            <div
              ...
              :class="
                store.activeLayerId === layer.id
                  ? 'bg-white/5 border-studio-primary text-white'
                  : 'bg-transparent'
              "
            >
              <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                  <div
                    class="w-8 h-8 rounded-lg bg-slate-800 flex items-center justify-center text-[10px] font-black text-slate-500"
                  >
                    #0{{ layer.id }}
                  </div>
                ...
                >
                  <button
                    ...
                    class="p-1.5 hover:text-white transition-colors"
                  ...
                  <button
                    class="p-1.5 hover:text-white transition-colors text-slate-600"
        ...
        <div class="mt-auto space-y-6 pt-10 border-t border-white/5 text-white">
          <div class="space-y-4">
            <div
              class="flex justify-between text-[10px] font-black uppercase text-slate-600 tracking-widest"
            >
            ...
            <input
              ...
              class="w-full h-1 bg-white/10 rounded-full appearance-none accent-studio-primary cursor-pointer"
              aria-label="Brush size"
            />
          </div>
          <button
            @click="store.clearCanvas"
            class="w-full py-4 glass-toolbar border-white/10 text-red-500 font-black uppercase tracking-widest text-[10px] hover:bg-red-500/10 transition-all flex items-center justify-center"
